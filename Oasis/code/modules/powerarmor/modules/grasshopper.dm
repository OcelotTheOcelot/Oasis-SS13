/obj/item/power_armor_module/grasshopper
	...
	var/jumpspeed = 1 // The speed of the hop
...
/obj/item/power_armor_module/grasshopper/proc/hop()
	...
	if(user.throw_at(target, jumpdistance, jumpspeed, spin = FALSE, diagonals_first = TRUE))
		...
		user.visible_message("<span class='warning'>[user] leaps forward using their [src]!</span>")
...
/datum/action/innate/power_armor/module/grasshopper_hop
	name = "Toggle hopping mode"
	desc = "Toggle the AJM-22 module, so the next your step will be a leap. Chasms are no longer a problem!"
